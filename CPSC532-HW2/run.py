...
from general_sampling import get_sample, prior_samples, posterior_samples
...
def run_programs(programs, mode, prog_set, base_dir, daphne_dir, num_samples=int(1e3), tmax=None, compile=False, wandb_run=False, verbose=False,):
    ...
    for i in programs:
        ...
        ast_or_graph = create_class(ast_or_graph, mode)
        samples, weights = posterior_samples(ast_or_graph, mode, num_samples, tmax=tmax, wandb_name=wandb_name, verbose=verbose)
        samples = tc.stack(samples).type(tc.float)
        weights = tc.stack(weights).type(tc.float)

        np.savetxt(results_file_samples(i), samples)
        np.savetxt(results_file_weights(i), weights)
...
def run_all(cfg):
    ...
    # Initialize W&B
    if wandb_run: wandb.init(project=prog_set +'-'+mode, entity='cs532-2022')
    ...
    run_programs(programs, mode=mode, prog_set=prog_set, base_dir=base_dir, daphne_dir=daphne_dir, num_samples=num_samples, 

...
from evaluation_based_sampling import abstract_syntax_tree, Eval
from graph_based_sampling import graph, standard_env
...
def run_all(cfg):
    ...
    if bbvi:
        for program in programs:
            ...
            print("\n The resulting approximating distribution is : {} \n".format(Q))
            ...
            if wandb_run:
                print("Sampling from Q...\n")
                if program==1 : 
                    samples = list(zip(*[list(Q[x].sample((int(1e5),))) for x in Q.keys()]))

                elif program==2 :
                    slope, bias = [Q[x].sample((int(1e5),)) for x in Q.keys()]
                    samples = list(zip(*[slope, bias, 10.0 * slope + bias]))
                
                elif program==3:
                    N = 10000
                    samples = tc.stack([Q["sample7"].sample() == Q["sample9"].sample() for _ in range(N)]).float()
                    print("posterior probability that the first and second datapoint are in the same cluster : {}".format(samples.mean()))
                
                elif program==4:
                    json_prog = './programs/HW4/4_graph.json'
                    ...
                    for j in range(1000):
                        env = standard_env()
                        env.update({x : Q[x].sample() for x in Q.keys()})
                        
                        sigma = {"logW":tc.tensor(0.0)}
                        ret, sigma = Eval(g.Program, sigma, env)
                        samples.append(ret)
                    ...
                    samples = tc.stack(samples).float()

                elif program==5:
                    samples = Q["sample2"].sample((10000,))
                else:
                    raise Exception("Program {} sampling not implemented.".format(program))
